> **Historical archive:** this is the former repository README, retained on
> 2026-09-13. It mixes club and personal robot details and includes superseded
> calibration values. Use the [robot records](../../robots/README.md) for current
> scope and specifications. Shell examples below assume the repository root.

# Robomo.club Robot 2024

Code and documentation for Robomo.club 2019-2024 club robot.

**TX2 wiring correction (user-confirmed 2026-09-12):** the Teensy on `tx2.local`
is powered through USB only; it has no separate supply. The older club/Nano
power diagrams below describe a different robot and must not be used to infer
the TX2's Teensy power wiring.
...
Every ROS container loads `fastdds_udp_only.xml` (via `FASTRTPS_DEFAULT_PROFILES_FILE`),
which turns off Fast DDS's shared-memory transport. With `ipc: host`, SHM segments in
`/dev/shm` outlived container restarts and a node starting on a stale one would OOM or hang
(typically rosbridge, so port 9090 never opened). UDP-only leaves nothing to go stale.
...
| `ros2_face` | [docker-ros2-face](https://github.com/frankjoshua/docker-ros2-face) | Animated browser face (port **8080**) driven by `/face/expression`, `/face/gaze`, and `/face/mouth`. |
...
| `ros2_diagnostics` | [diagnostics/](../../diagnostics) (vanilla `ros:humble-ros-base`) | Publishes `/diagnostics` — one OK/WARN/ERROR status per system with live readings. `teensy` reads `/vel` + `/joint_states` (wheel angle, RPM, motor power). |
...
The face starts with the core stack, including `./start_mock.sh` and `./start_ros.sh`.
Open `http://localhost:8080/` on the machine running the stack. From another laptop,
use the robot's address: `http://robmo-club-robot.local:8080/` for the club robot,
or `http://tx2.stork-spica.ts.net:8080/` for the home TX2 over Tailscale.
`localhost` always refers to the machine running the browser. On the robot's monitor,
open the face page in a browser's fullscreen or kiosk mode.

From a ROS 2 shell on the same DDS domain, control the face with:

```bash
ros2 topic pub --once /face/expression std_msgs/msg/String '{data: happy}'
ros2 topic pub --once /face/gaze geometry_msgs/msg/Point '{x: 0.8, y: 0.2, z: 0.0}'
ros2 topic pub --rate 20 /face/mouth std_msgs/msg/Float32 '{data: 0.7}'
```

Expressions: `neutral`, `happy`, `sad`, `surprised`, `angry`, `sleepy`. Gaze coordinates
range from -1 to 1 (+x right, +y up). Mouth opening ranges from 0 to 1 and returns
to closed when updates stop for 300 ms.
...
```text
  ...
  (runs the containers listed under "Containers" above)
    ...
    └─ USB ─┬─ Teensy 4.0 ........ /dev/teensy   (16c0:0483)   drive motors + wheel encoders
            ...
            ├─ Intel RealSense D435i (8086:0b3a)                depth + color + IMU → Nav2 obstacles
...
| Intel RealSense D435i | Intel `8086:0b3a` | USB 3 | `ros2_realsense` | Depth + color + IMU; depth feeds Nav2 obstacle avoidance |
...
at 9600 baud in Sabertooth "Simplified Serial"** — one byte per side. On the TX2 robot,
the physical left wheel is M2 (bytes 128–255), and the right wheel is M1 (bytes 1–127).
The wheel-side correction also swaps encoder feedback and migrates saved calibration;
positive `angular.z` commands left backwards and right forwards (counterclockwise).
It reads the wheel encoders through the dual
...
```text
  /cmd_vel ─► micro_ros_agent ─► Teensy 4.0 ───────────────► Sabertooth ─► L / R motors
   ...
   /odom ◄── wheel_imu_odometry ◄── /vel + /camera/camera/imu
```

TX2 rolling calibration, saved in Teensy EEPROM and [config/base-calibration.json](../../config/base-calibration.json).
See [the calibration report](../../robots/personal/docs/odometry-calibration.md) for measurements and limitations.

| Parameter | Value |
|-----------|-------|
| Drive base | Zagros MAX-14 round chassis, identified by the owner; installed motor variant unverified |
| Base type | differential drive |
| Wheel diameter | 0.1524 m (6 inches) |
| Effective wheel track | 0.30 m (rolling estimate) |
| Max motor RPM | 80 |
| Effective encoder counts / rev (x4) | 310000 (rolling estimate, not a counted shaft revolution) |

The 80 RPM setting is retained; it is not a measured maximum wheel speed.
Odometry uses the RealSense gyro for heading, with stationary bias correction and
wheel-heading fallback if IMU data is stale. `/diagnostics` reports which source is active.
...
installed encoder and motor part numbers have not been verified. The effective
count scale was measured against laser motion; do not infer the hardware model from it.
...
```text
  map ─► odom ─► base_link ─┬─ base_footprint   ( 0.00,  0.00, 0.00 )  ground projection
   (slam) (diff_drive)      ├─ laser_frame      ( 0.03,  0.00, 1.23 )  YDLidar, top of pole
                            ├─ camera_link      ( 0.31,  0.00, 0.42 )  RealSense, floor-aligned
...
| `camera_link` | `base_link` | 0.31, 0, 0.42 | D435i; floor-estimated roll/pitch 0.0193/0.0208 rad |
...
RealSense deployment, stream checks, Nav2 depth layers, and rollback details are in
[RealSense obstacle avoidance](../../robots/personal/docs/realsense-obstacles.md).
